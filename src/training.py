    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            if not best_acc or test_result.accuracy > best_acc:
                save_checkpoint = True

            if epoch == 0:
                current_min_loss = 0
            elif test_loss[current_min_loss] <= test_loss[epoch] + 1e-5:
                epochs_without_improvement += 1
            else:
                epochs_without_improvement = 0
                current_min_loss = epoch

            if (early_stopping is not None and epochs_without_improvement >= early_stopping) or best_acc == 100:
                break
            # ========================
...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ========================
        yt, ht = self.model(x, self.hidden_state)
        loss = self.loss_fn(input=torch.transpose(yt, dim0=1, dim1=2), target=y)
        self.hidden_state = torch.detach(ht)
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        with torch.no_grad():
            ...
            # ========================
            yt, self.hidden_state = self.model(x, self.hidden_state)
            loss = self.loss_fn(input=torch.transpose(yt, dim0=1, dim1=2), target=y)
